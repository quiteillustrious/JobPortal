...
switch ($request) {
    ...
    case "republishpublication":

        $update = execsqlSRS("UPDATE [tbl_Publication] SET [pubstatus_id] = '5' WHERE [publication_id] = ?", "Update", [$datavalue]);

        echo json_encode(["status" => "success", "message" => "Publication republished successfully."]);
